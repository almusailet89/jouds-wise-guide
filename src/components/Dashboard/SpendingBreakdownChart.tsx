...
export const SpendingBreakdownChart: React.FC<SpendingBreakdownChartProps> = ({ entries, currency }) => {
  const data = useMemo(() => {
    ...
    if (sorted.length <= 7) return sorted.map(([name, value]) => ({ name: CAT_AR[name] || name, value, rawName: name }));
    const top6 = sorted.slice(0, 6);
    const otherSum = sorted.slice(6).reduce((s, [, v]) => s + v, 0);
    return [
      ...top6.map(([name, value]) => ({ name: CAT_AR[name] || name, value, rawName: name })),
      { name: 'أخرى', value: otherSum, rawName: 'other' },
    ];
  }, [entries]);
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    ...
    return (
      <div className="bg-card/95 backdrop-blur border border-border/50 rounded-xl p-3 shadow-elegant" dir="rtl">
        ...
        <p className="text-xs text-muted-foreground font-arabic">{pct}٪ من المصاريف</p>
      </div>
    );
  };

  const CustomLegend = ({ payload }: any) => (
    <div className="flex flex-wrap justify-center gap-x-4 gap-y-1.5 mt-3" dir="rtl">
      ...
      <CardContent>
        {data.length > 0 ? (
          ...
          <div className="h-64 flex flex-col items-center justify-center text-center gap-2">
            ...
            <p className="font-arabic text-sm text-muted-foreground">لا توجد مصاريف في هذه الفترة</p>
            <p className="font-arabic text-xs text-muted-foreground/60">
              قولي لجود: «صرفت ٢٠٠ ريال على الغداء»
            </p>
          </div>
        )}
      </CardContent>
