...
const functionTools = [
  ...
  { type: "function", function: { name: "create_calendar_event", description: "Create calendar event/meeting/appointment/block time. Trigger: 'احجزي', 'اجتماع', 'موعد', 'حطّي في التقويم', 'جدولي', 'حطّي بلوك', 'عندي لقاء', 'موعد دكتور', 'رحلة', 'حفلة', 'مناسبة', 'book', 'schedule a meeting', 'block time for', 'set up a call', 'board meeting', 'doctor appointment', 'dinner reservation', 'flight', 'conference', 'interview'. Also for personal: 'عزيمة', 'زيارة', 'مشوار'. COMPOUND BOOKING RULES: (a) 'book X with reminder Y min' → set reminder_min=Y. (b) 'book X with note/notes Y' → set description=Y. (c) 'book X as project/follow up/track' → set category='project', and ALSO call add_task with title='متابعة: X' / 'Follow up: X' and same date. (d) Multiple instructions in one message (reminder + note + project) → single create_calendar_event call with all relevant fields filled — NEVER ask the user to repeat or split.", parameters: { type: "object", properties: { title: { type: "string" }, starts_at: { type: "string" }, ends_at: { type: "string" }, location: { type: "string" }, description: { type: "string", description: "Notes or details attached to this event." }, all_day: { type: "boolean" }, category: { type: "string", description: "Event category. Use 'project' when user says 'as project', 'follow up', 'track', 'متابعة', 'مشروع'." }, reminder_min: { type: "number", description: "Minutes before event to remind. Parse from user: 'remind me 30 min before'→30, 'ذكّريني قبل ساعة'→60, 'no reminder'→0. Default 15 if user says 'with reminder' without specifying." } }, required: ["title","starts_at"] } } },
...
async function executeFunction(functionCall: any, userId: string, supabase: any) {
  ...
  switch (name) {
    ...
    case 'create_calendar_event': {
      ...
      // Auto-create a follow-up task when booked as a project
      let projectTaskNote = '';
      if (args.category === 'project') {
        const followTitle = `متابعة: ${args.title}`;
        const followDate = startsAt.slice(0, 10);
        await supabase.from('tasks').insert({ user_id: userId, title: followTitle, due_date: followDate, priority: 'high', description: args.description || null, status: 'pending', category: 'project' }).catch(() => {});
        projectTaskNote = ' + أضفت مهمة متابعة في قائمة المهام';
      }

      const reminderNote = args.reminder_min !== undefined && args.reminder_min !== 15 ? ` · تذكير قبل ${args.reminder_min} دقيقة` : '';
      const descNote = args.description ? ` · ملاحظة: "${args.description.slice(0, 40)}"` : '';
      return { kind: 'event', summary: `✓ سجّلت موعد "${args.title}" يوم ${evtFmt}${args.location ? ` في ${args.location}` : ''}${reminderNote}${descNote}${projectTaskNote}`, data: args };
    }
    case 'compose_email':
...
لا تستخدمي صيغة المذكر أبداً معها.`;
      if (g === 'male') return `
🔴 قاعدة الجنس (إلزامية): المستخدم ذكر — استخدمي صيغة المذكر في كل ردودك بدون استثناء.
أمثلة صحيحة: "قل، أخبرني، أنتَ، تفضّل، سجّل، افعل، هل أنت متأكد".
ممنوع منعاً باتاً: "قولي، أخبريني، أنتِ، تفضّلي، سجّلي، افعلي، متأكدة".
هذه القاعدة تطغى على كل شيء — لا استثناءات.`;
...
📅 الحجوزات المركّبة (كل شي في رسالة واحدة — لا تسألي مرة ثانية):
• "احجزي اجتماع الساعة ٣ وذكّريني قبل ساعة" → create_calendar_event(reminder_min=60)
• "حطّي موعد الدكتور مع ملاحظة 'جيب الملف'" → create_calendar_event(description='جيب الملف')
• "احجزي هذا كمشروع للمتابعة" → create_calendar_event(category='project') + add_task(title='متابعة: [اسم الحدث]', same date, priority=high)
• "احجزي اجتماع بكرة ١٠ص مع تذكير ٣٠ دقيقة وملاحظة 'جهّز الشرائح'" → create_calendar_event(reminder_min=30, description='جهّز الشرائح') — call once, fill all fields
• "book dentist Thursday 2pm remind me 1 hour before, note: bring X-rays, track as project" → create_calendar_event(reminder_min=60, description='bring X-rays', category='project') + add_task(title='Follow up: dentist')
• "ذكّريني بـ X" بدون موعد → add_task (مو حدث تقويم)
• أي تعليمات متعددة في جملة واحدة → نفّذيها كلها دفعة واحدة بدون ما تسألي
